  ...
  return (
    <div className="set">
      ...
      <div className="btn-row mt-14">
        <button className="btn ghost" disabled={demo} onClick={() => fileRef.current?.click()}>
          CSV 업로드
        </button>
        <button className="btn ghost" disabled={demo} onClick={() => setPasteMode(!pasteMode)}>
          붙여넣기
        </button>
      ...
      {pasteMode && (
        <>
          <textarea
            placeholder={"김예찬,19940101,010-1234-5678\n이요셉,19920302,010-2222-1234"}
            value={pasteText}
            onChange={(e) => setPasteText(e.target.value)}
          />
          <button className="btn sm ghost mt-8" onClick={() => handleText(pasteText)}>
            확인
          </button>
        </>
      )}
        ...
        <div className="msg mt-14">
          ...
          {preview.rows.length > 0 && (
            <>
              ...
              <button className="btn sm accent block mt-10" disabled={pending} onClick={submit}>
      ...
      {result && <p className={`msg ${result.ok ? "ok" : "err"}`}>{result.message}</p>}
      <small className="mt-12">
        같은 이름+생년월일+전화번호는 갱신, 새 조합은 추가돼요. 기존 체크인·바인딩은
        유지됩니다.
      </small>
    </div>
  );
