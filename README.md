...
참가자 페이지는 전부 **사전 렌더링(정적/ISR)**이라 Vercel CDN 엣지에서 즉시 서빙되고,
Link가 뷰포트에 들어오면 자동 prefetch되어 클릭 시 서버 왕복 없이 전환된다.

| 라우트 | 방식 |
|---|---|
| `/` `/songs` | ISR 60초 |
| `/guestbook` | ISR 30초 |
| `/about` `/speakers` `/timetable` | ISR 1시간 |
...
전환 표시는 `useLinkStatus`로 클릭한 링크에만 준다(페이지를 fallback으로 갈아끼우지
않으므로 푸터가 튀지 않는다).

함수 리전은 [vercel.json](vercel.json)에서 서울(`icn1`)로 고정한다 — 기본값은
워싱턴DC라 한국에서 동적 라우트·API 호출이 태평양을 왕복한다.

내비게이션은 화면 폭에 따라 갈린다. **데스크톱(768px~)은 상단 가로 메뉴**,
**모바일은 하단 탭바**(홈·일정·강사·찬양·My)로 행사 당일 체크인 QR 접근을 최단화했다.
가로 여백은 `--gutter` 하나로 통일했다. **데스크톱은 0** — 컨테이너(640px) 경계가 포스터 좌우 끝과
정확히 맞아 상단 내비 로고까지 같은 선에 선다. **모바일은 16px**이고, 카드·그리드·미디어 블록
(강사 그리드·장소 카드·플레이어·갤러리·My 카드 등)은 음수 마진으로 화면 끝까지 늘어나 포스터처럼
꽉 찬다. 본문 텍스트만 여백을 유지해 가독성을 지킨다.
