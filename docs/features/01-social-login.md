...
Supabase에는 이를 위한 **Manual Linking** API(`linkIdentity`)가 있고,
켜면 `auth.identities`에 두 provider를 한 유저에 붙일 수 있다.

현재는 미구현이다. 실제로 문제가 되는 경우는 한 사람이 두 계정으로 가입 요청을 보내는 것인데,
`bind_participant`가 `taken`으로 막고 관리자가 체크인 탭에서 연결 해제로 정리할 수 있다
→ [04. 체크인](04-checkin.md)

구현하려면:

1. Supabase Dashboard → Authentication → **Manual Linking 활성화**
2. My 화면에 "로그인 수단 추가" 버튼 → `supabase.auth.linkIdentity({ provider: "google" })`
